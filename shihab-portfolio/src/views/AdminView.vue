...
const { signedIn, authReady, adminEmail, logout } = useAuth()
...
<template>
  <!-- One stable root element, deliberately. This view is lazy-loaded, so the
       <Transition> in App.vue is still mid-enter when onAuthStateChanged flips
       `authReady` below. If that swapped the component's ROOT element, the new
       element would keep the `page-enter-from` class (opacity: 0) and never get
       the frame that removes it — a blank admin page. Swapping inside a fixed
       root keeps the transition pointed at an element that does not move. -->
  <div>
    <!-- Firebase resolves the stored session asynchronously; without this gate a
         reload on /#/admin flashes the sign-in form for a frame. -->
    <div v-if="!authReady" class="grid min-h-screen place-items-center bg-ink-950 px-5 text-sm text-ink-400">
      Checking your session…
    </div>

    <AdminLogin v-else-if="!signedIn" />
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-[11px] text-ink-500">
                ...
                <span v-if="adminEmail" class="hidden sm:inline">· {{ adminEmail }}</span>
